...
class LKStar(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=13):
        super().__init__()
        mid_channels = out_channels // 2  # Pastikan jumlah channel sesuai
        self.lkconv1 = LKConv(in_channels, mid_channels, kernel_size)
        self.lkconv2 = LKConv(mid_channels, mid_channels, kernel_size)
        self.pwconv = nn.Conv2d(mid_channels * 2, out_channels, 1, bias=False)  # Pastikan jumlah channel sesuai
        self.bn = nn.BatchNorm2d(out_channels)
        self.silu = nn.SiLU()
    ...
    def forward(self, x):
        x1 = self.lkconv1(x)
        x2 = self.lkconv2(x1)
        out = torch.cat((x1, x2), dim=1)  # Gabungkan fitur sebelum masuk ke pwconv
        out = self.pwconv(out)
        out = self.bn(out)
